...
export async function GET(
  ...
  const [clock] = await db.select().from(worldClock).where(eq(worldClock.projectId, projectId));
  const events = await db.select().from(betweenChapterEvents).where(eq(betweenChapterEvents.projectId, projectId));
